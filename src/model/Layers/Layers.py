...
class ConvGRU2DCell(Layer):
  """Cell class for the ConvGRU layer.
  ...
  def __init__(self,
    ...
    self.state_size = self.filters# modified
    self._dropout_mask = None
    self._recurrent_dropout_mask = None
...
class TrajGRUCell(Layer):
  """Cell class for the TrajGRU layer.
  ...
  def __init__(self,
               ...
               reset_after=False,
               **kwargs):
    super(TrajGRUCell, self).__init__(**kwargs)
    self.filters = filters
    self.kernel_size = conv_utils.normalize_tuple(kernel_size, 2, 'kernel_size')
    self.strides = conv_utils.normalize_tuple(strides, 2, 'strides')
    self.padding = conv_utils.normalize_padding(padding)
  ...
  def get_config(self):
    config = {'filters': self.filters,
              ...
              'recurrent_dropout': self.recurrent_dropout,
              'reset_after': self.reset_after}
    base_config = super(TrajGRUCell, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))